...
namespace SemanticBUApp
{
    public partial class Form1 : Form
    {
        ...
        public Form1(string name)
        {
            ...
            tb_targetPath.Text = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "Semantic Backup", lblUsername.Text);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
           ...
           //sends table data for archives:
            ...
            {
                Directory.CreateDirectory(backupInfoFolderPath);
            }

            string backupInfoFilePath = Path.Combine(backupInfoFolderPath, tb_customBUname.Text + ".txt");
            using (StreamWriter writer = new StreamWriter(backupInfoFilePath))
            {
                // Write backup details to the file
                writer.WriteLine($"Backup Name: {tb_customBUname.Text}");
                writer.WriteLine($"Backup Location: {targetDir}");
                writer.WriteLine($"Backup Size: {new FileInfo(backUpFileName).Length / 100000} MB");
                writer.WriteLine($"Backup Date: {DateTime.Now}");
                writer.WriteLine("-------------------------------------------------------------------------------------------------------");
           
                // Write backup details to the file
                foreach (string file in approvedFiles)
                {
                    FileInfo fileInfo = new FileInfo(file);
                    writer.WriteLine($"File Name: {fileInfo.Name}");
                    writer.WriteLine($"File Path: {file}");
                    writer.WriteLine($"File Size: {fileInfo.Length / 100000} MB");
                    writer.WriteLine($"Backup Date: {DateTime.Now}");
                    writer.WriteLine();
                }
            }
            ...
            MessageBox.Show($"Backup information saved to: {backupInfoFilePath}");
